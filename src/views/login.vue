  ...
  proxy.$refs.loginRef.validate((valid: boolean) => {
    if (valid) {
      ...
      if (loginForm.value.rememberMe) {
        Cookies.set("username", loginForm.value.username, { expires: 30, secure: location.protocol === 'https:' })
        Cookies.set("password", encrypt(loginForm.value.password), { expires: 30, secure: location.protocol === 'https:' })
        Cookies.set("rememberMe", loginForm.value.rememberMe, { expires: 30, secure: location.protocol === 'https:' })
      } else {
        // 否则移除
