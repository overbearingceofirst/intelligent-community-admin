> **Note (original repo):** intelligent-community-admin

# intelligent-community-admin — Vue3 前端管理模板

> 本项目基于 [RuoYi-Vue3 (typescript 分支)](https://gitcode.com/yangzongzhuan/RuoYi-Vue3/tree/typescript) 集成，已适配 [intelligent-community-node](https://github.com/overbearingceofirst/intelligent-community-node) Node.js 后端。
...
# 3. 本地开发（开发环境会通过 vite proxy 将 /api 转发到 http://localhost:3000）
npm run dev
...
- **Vite 代理**：`vite.config.ts` 中配置 `/api` → `http://localhost:3000`，开发时无需修改后端 CORS。
- **环境变量**：`.env.development` 设置 `VITE_API_BASE=http://localhost:3000/api`；`.env.production` 设置 `VITE_API_BASE=/api`。
- **Axios 适配**：`src/utils/request.ts` 中：
  - `baseURL` 使用 `import.meta.env.VITE_API_BASE || '/api'`
  - 请求拦截器自动从 `localStorage` 注入 `token`（`Authorization: Bearer <token>`）
  - 响应拦截器适配后端格式 `{ code, msg, data }`；`code===200` 返回 `data`；`code===401` 清除 token 并跳转 `/login`
- **构建脚本**：`package.json` 增加 `build` 和 `lint` 脚本以便 CI 调用。

## 来源声明 (Source Attribution)

前端源码来自 **RuoYi-Vue3 (typescript 分支)**：
- 地址：https://gitcode.com/yangzongzhuan/RuoYi-Vue3/tree/typescript
- 镜像：https://github.com/yangzongzhuan/RuoYi-Vue3/tree/typescript
- License：MIT

## 后续建议 (Next Steps)

- 对齐 API 字段：根据 `intelligent-community-node` 实际接口调整 `src/api/` 中的接口定义（登录、菜单、用户、社区管理等）
- 代码规范：添加 `husky` + `lint-staged` 以在提交前自动执行 lint
- CI/CD 部署：完善 `.github/workflows/ci.yml`，增加镜像构建推送、自动部署到服务器
- 国际化：集成 `vue-i18n` 实现多语言支持
- 权限路由：对接 `intelligent-community-node` 的 `/api/getRouters` 或等效接口，动态加载菜单
- 环境区分：按需增加 `.env.staging` 等环境配置

---

> 以下为 RuoYi-Vue3 原始 README：
